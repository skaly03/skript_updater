# GitHub: https://github.com/tve/mqboard/tree/master/mqtt_async
...
config = {
    ...
    "ssid": "No_SSID_DEFINED", # has to be something, but not none! -> Exception on line 575 / 579 will raise - In case of usage without micropython
    "wifi_pw": None,
...
class MQTTClient:
    ...
    async def connect(self):
        ...
        if not self._c["interface"].isconnected():
            # use own wifi handling if available
        ...
        if self._c["connect_coro"] is not None:
            ...
            log.info("Subscribing specified topics successful")
    ...
    async def _keep_connected(self):
        while self._state <= 1:
            ...
            # reconnect to Wifi
            elif not self._c["interface"].isconnected():
                try:
                    loop = asyncio.get_event_loop()
                    if self._c["wifi_coro"] is not None:
                        loop.create_task(self._c["wifi_coro"](True))
